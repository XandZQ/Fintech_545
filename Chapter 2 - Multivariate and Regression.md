...
> 分母用 $n-1$ 而非 $n$（Bessel's correction），这样是 unbiased estimator。
...
在 risk management 中，Spearman 更常用于 copula fitting（Chapter 5），因为金融数据 rarely Gaussian。
...
> 如果估计出的 covariance matrix 不是 PSD（比如 pairwise 估计 + missing data），需要用 Higham / near_psd 修正（详见 Chapter 3 笔记）。
...
> 性质 4 是 Gaussian 世界的"便利性"：correlation = dependence。但真实市场中不成立，这是 copula（Chapter 5）存在的理由。
...
详细的 Cholesky 推导和 3×3 数值例子在 Chapter 3 笔记中。
...
- **条件均值** $\mu_{2|1}$：原始均值 $\mu_2$ 加了一个 **调整项** $\frac{\sigma_{12}}{\sigma_1^2}(x_1 - \mu_1)$
  ...
  - 调整项 = $\rho \cdot \frac{\sigma_2}{\sigma_1} \cdot (x_1 - \mu_1)$（这就是 regression 的系数！）
...
这在实际金融数据中更常用，因为 returns 通常有 heavy tails（excess kurtosis > 0）。
...
GARCH 让方差本身变成一个 time-varying process。
