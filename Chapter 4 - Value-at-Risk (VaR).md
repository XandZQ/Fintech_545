...
> VaR 本质上就是 **CDF 的反函数**，在 Chapter 1 中我们已经学过 quantile function 了，这里只是应用到了 loss 的语境。
...
由 multivariate normal 的性质（Chapter 2 已推导）：
...
- 可以生成 tail scenarios
- 在 Chapter 3 中我们已经学了 simulation 的底层工具（Cholesky、PCA）
...
在 Chapter 3 中我们已经推导过 exponentially weighted variance：
...
- **Chapter 1**：Quantile function → VaR 就是 quantile 的应用
- **Chapter 2**：$w'\Sigma w$ → Portfolio VaR 的核心公式来自 covariance matrix
- **Chapter 3**：Monte Carlo simulation → VaR 的第三种计算方法；Exponentially weighted covariance → VaR 估计中的权重方案
